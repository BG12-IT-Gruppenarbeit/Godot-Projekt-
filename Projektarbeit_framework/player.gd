...
var held_wpns : Array = ["shotgun","pistol"]	#array of current weapons | held_wpns[0] is primary and 1 is secondary
...
func _process(delta: float) -> void:
	if $Shoot_timer.is_stopped():
		if Input.is_action_pressed("shoot"):
			weapon.shoot($Firepoint, dmg_up, dmg_mult, spread)	#shoots the current weapon
